    ...
    <section className="py-20 md:py-28 bg-bg-alt" ref={ref}>
      <div className={`container mx-auto px-4 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((p, i) => (
            <div key={i} className="glass rounded-2xl p-6 md:p-8 flex flex-col hover:border-accent/30 hover:shadow-[0_0_30px_rgba(242,140,40,0.1)] transition-all duration-300 group" style={{ transitionDelay: `${i * 100}ms` }}>
              ...
              <ul className="space-y-2 mb-6 flex-1">
                {p.items.map((item) => (
                  <li key={item} className="flex items-center gap-2 text-sm font-inter text-foreground/80">
                    <Check size={14} className="text-accent shrink-0" /> {item}
                  </li>
                ))}
              </ul>
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="w-full bg-accent text-accent-foreground py-3 text-center font-semibold font-inter rounded-lg hover:brightness-110 transition-all min-h-[48px] flex items-center justify-center">
                {p.cta}
              </a>
            </div>
          ))}
